...
namespace Couriers.Speedex.Tests
{
    ...
    public sealed class BaseSpeedexClientUnitTests : IAsyncLifetime, IDisposable
    {
        ...
        /// <summary>
        /// Validates that when <see cref="BaseSpeedexClient"/> constructors are called, 
        /// with <see langword="null"/>, an <see cref="Exception"/> is thrown
        /// </summary>
        /// <returns></returns>
        [Fact]
        public void BaseSpeedexClient_WithNullData_ThrowsException()
        {
            Assert.ThrowsAny<Exception>(() => new DemoSpeedexClient(null!));

            Assert.ThrowsAny<Exception>(() => new DemoSpeedexClient(TestConstants.SpeedexCredentials, null!));
        }
