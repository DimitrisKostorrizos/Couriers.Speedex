...
namespace Couriers.Speedex.Tests
{
    ...
    public sealed class SpeedexHelpersUnitTests
    {
        ...
        /// <summary>
        /// Validates that when <see cref="SpeedexHelpers.GetDeliveryTimeLimitByTimeRange(DateTime?, DateTime?)"/> method is called, 
        /// using every available value for <see cref="DeliveryTimeLimit"/>, no exception is thrown
        /// </summary>
        /// <returns></returns>
        [Fact]
        public void GetDeliveryTimeLimitByTimeRange_AllAvailableArguments_NoExceptionIsThrown()
        {
            ...
            deliveryTimeLimit = SpeedexHelpers.GetDeliveryTimeLimitByTimeRange(DateTime.MinValue.AddHours(10), DateTime.MinValue.AddHours(13));

            Assert.Equal(DeliveryTimeLimit.TenAMToOnePM, deliveryTimeLimit);

            deliveryTimeLimit = SpeedexHelpers.GetDeliveryTimeLimitByTimeRange(DateTime.MinValue.AddHours(13), DateTime.MinValue.AddHours(16));

            Assert.Equal(DeliveryTimeLimit.OnePMMToFourPM, deliveryTimeLimit);

            deliveryTimeLimit = SpeedexHelpers.GetDeliveryTimeLimitByTimeRange(DateTime.MinValue.AddHours(16), DateTime.MinValue.AddHours(19));

            Assert.Equal(DeliveryTimeLimit.FourPMToSevenPM, deliveryTimeLimit);

            Assert.ThrowsAny<Exception>(() => SpeedexHelpers.GetDeliveryTimeLimitByTimeRange(null, DateTime.MinValue.AddHours(19)));
        }
